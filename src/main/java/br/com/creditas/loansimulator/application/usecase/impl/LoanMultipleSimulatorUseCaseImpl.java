...
public class LoanMultipleSimulatorUseCaseImpl implements LoanMultipleSimulatorUseCase {
    ...
    public LoanMultipleSimulatorUseCaseImpl(LoanSimulatorUseCase loanSimulatorUseCase,
    ...
    public CompletableFuture<List<LoanSimulation>> execute(List<LoanSimulation> simulationsList) {
        ...
        List<CompletableFuture<LoanSimulation>> futures = simulationsList.stream()
                .map(simulation -> CompletableFuture.supplyAsync(
                        () -> loanSimulatorUseCase.execute(simulation),
                        batchModeTaskExecutor
                ))
                .toList();
