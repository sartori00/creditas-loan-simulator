...
public class LoanController {
    ...
    public CompletableFuture<ResponseEntity<List<LoanSimulationResponseDto>>> simulateMultipleLoans(
        ...
        List<LoanSimulation> simulationsToProcess = dto.stream()
                .map(LoanSimulationRequestDto::toModel)
                .toList();
        ...
        CompletableFuture<List<LoanSimulationResponseDto>> responseDtosFuture =
                simulationsFuture.thenApply(simulations ->
                        simulations.stream()
                                .map(loanSimulation -> {
        ...
        CompletableFuture<ResponseEntity<List<LoanSimulationResponseDto>>> finalResponseEntityFuture =
                responseDtosFuture.thenApply(response -> ResponseEntity.status(HttpStatus.OK).body(response));

        finalResponseEntityFuture.whenComplete((responseEntity, throwable) -> {
            if (throwable == null) {
